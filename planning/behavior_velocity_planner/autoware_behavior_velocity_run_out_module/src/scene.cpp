  ...
  std::vector<geometry_msgs::msg::Point> & collision_points) const
{
  ...
  for (const auto & point : bg_ranged_polygon.outer()) {
    const auto p_msg =
      autoware::universe_utils::createPoint(point.x(), point.y(), pose_with_range.pose_min.position.z);
    ranged_polygon.emplace_back(p_msg);
  }

  if (ranged_polygon.empty()) {
    RCLCPP_WARN_STREAM(logger_, "failed to check collision with polygon");
    return false;
  }
